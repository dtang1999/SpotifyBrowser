  ...
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            let curr1 = predictions[0].bbox[0];
            let curr2 = predictions[0].bbox[2];
            if (openhands == 1)
            {
              if (curr1 > this.x1_prev && curr2 > this.x2_prev)
              {
                console.log("right swipe successfully")
                this.detectedGesture = "Right Swipe";
              }
              else if (curr1 < this.x1_prev && curr2 < this.x2_prev)
              {
                this.detectedGesture = "Left Swipe";
                console.log("left swipe successfully")
              }
              else
              {
            ...
            //Disable the Detect Camera with Two Closed Hands
            if (this.detectedGesture == "Two Closed Hands")
            {
              this.stopDetection();
            }
